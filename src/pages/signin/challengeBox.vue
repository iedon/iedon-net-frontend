...
import { AuthRequestResponse } from '../../common/packetHandler'
...
const data = computed(() => props.authRequestResp)
        ...
        >
            <a-collapse accordion v-model:activeKey="activeKey">
                <a-collapse-panel key="challengeHint">
                    ...
                    <span style="word-break:break-all;font-size:12px;user-select:text">{{ props.authRequestResp?.authChallenge }}</span>
                </a-collapse-panel>
            </a-collapse>
            <br />

            <a-form-item v-if="props.type === 'pgp-fingerprint'" name="publicKey" :label="`${t('pages.signIn.pgpPublicKey')}`"
                :rules="[{ required: true, message: `${t('pages.signIn.pleaseInput')} ${t('pages.signIn.pgpPublicKey')}` }]"
            >
                <a-textarea auto-size v-model:value="challengeForm.publicKey" :placeholder="`${t('pages.signIn.pgpPublicKey')}`" />
            </a-form-item>
            ...
            >
                <a-input-password v-if="props.type === 'password'" autocomplete="password" v-model:value="challengeForm.challengeText" :placeholder="t('pages.signIn.challengeTextPlaceholder')" />
                <a-textarea v-else auto-size v-model:value="challengeForm.challengeText" :placeholder="`${t('pages.signIn.challengeTextPlaceholder')}`" />
            </a-form-item>
