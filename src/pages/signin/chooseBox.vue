...
import { AuthQueryResponse } from '../../common/packetHandler'
...
<template>
    <a-spin :tip="`${t('pages.signIn.pleaseWait')}`" :spinning="isLoading">
        ...
        <br />
        <a-form :model="requestChallengeForm"
            :label-col="{ span: 8 }"
            :wrapper-col="{ span: 16 }"
        >
            <a-collapse accordion v-model:activeKey="activeKey">
                <template v-for="method in data?.availableAuthMethods" :key="`desc_${method.id}`">
                    <a-collapse-panel>
                        <template #header>
                            <b>{{method.id + 1}}</b><span style="margin-left:20px;font-style:italic">{{method.type}}</span>
                        </template>
                        <span style="word-break:break-all;font-size:12px;user-select:text">{{ method.data || (method.type === 'password' ? t('pages.signIn.useSitePassword') : '') }}</span>
                    </a-collapse-panel>
                </template>
            </a-collapse>
            <br />
            <a-form-item :label="t('pages.signIn.authenticateWith')">
                <a-radio-group v-model:value="requestChallengeForm.method">
                    <a-radio v-for="method in data?.availableAuthMethods" :key="`method_${method.id}`" :value="method.id">
                        {{ method.id + 1 }} ({{ method.type }})
                    </a-radio>
                </a-radio-group>
            </a-form-item>
            ...
            <a-form-item :wrapper-col="{ offset: 8, span: 16 }">
                ...
                <a-button type="primary" @click="props.requestChallenge(requestChallengeForm.method)" :disabled="loading">
                    <template #icon>
                        <send-outlined />
                    </template>
